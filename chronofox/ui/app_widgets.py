...
class RoundedWindow(QWidget):
    ...
    def drag_locked(self) -> bool:
        """P-D1: 핀 모드가 걸린 창(메인 달력)만 override해 드래그/리사이즈를 막는다.
        기본값 False — 다른 RoundedWindow 서브클래스(일정/메모/설정 등)는 이 훅을
        override하지 않으므로 기존 드래그 동작이 그대로 유지된다."""
        return False
...
class CalendarStyleButton(QPushButton):
    """R16: 기본(grid)/미니멀/셀 카드 중 하나의 달력 모양을 선택하는 아이콘 버튼입니다.

    ThemeButton과 같은 시각 패턴(체크 시 패널 배경, 아이콘+라벨)을 공유하되, 아이콘은
    calendar_style 프리셋별로 다르다.
    """

    LABEL_WIDTHS = {"grid": 88, "minimal": 96, "card": 104}

    def __init__(self, style: str, label: str, colors: dict[str, str]) -> None:
        super().__init__()
        self.style_id = style
        self.label = label
        self.colors = colors
        ...
        self.setCursor(Qt.PointingHandCursor)
        self.setFixedHeight(30)
        self.setFixedWidth(self.LABEL_WIDTHS.get(style, 96))
        self.setStyleSheet("QPushButton { border: none; background: transparent; }")
    ...
    def draw_icon(self, painter: QPainter, color: QColor) -> None:
        """달력 모양(기본/미니멀/셀 카드)을 나타내는 작은 아이콘을 그립니다."""
        pen = QPen(color, 1.15)
        pen.setCapStyle(Qt.RoundCap)
        pen.setJoinStyle(Qt.RoundJoin)
        painter.setPen(pen)
        ...
        if self.style_id == "grid":
            painter.drawRect(QRect(7, 6, 18, 18))
            painter.drawLine(7, 12, 25, 12)
            painter.drawLine(7, 18, 25, 18)
            painter.drawLine(13, 6, 13, 24)
            painter.drawLine(19, 6, 19, 24)
        elif self.style_id == "minimal":
            painter.drawRoundedRect(QRect(7, 6, 18, 18), 3, 3)
            painter.setPen(Qt.NoPen)
            painter.setBrush(color)
            for cx in (12, 16, 20):
                painter.drawEllipse(QPoint(cx, 19), 2, 2)
        elif self.style_id == "card":
            painter.drawRoundedRect(QRect(7, 6, 8, 8), 2, 2)
            painter.drawRoundedRect(QRect(17, 6, 8, 8), 2, 2)
            painter.drawRoundedRect(QRect(7, 16, 8, 8), 2, 2)
            painter.setPen(Qt.NoPen)
            painter.setBrush(color)
            painter.drawRoundedRect(QRect(17, 16, 8, 8), 2, 2)
