"""허브(R4-1)의 빈 골격 섹션(Today/알람/설정) 믹스인.

세 섹션 모두 아직 실제 콘텐츠가 없다 — 실이식은 각각 R4-5(Today)/R4-3(알람)/
R4-4(설정)에서 진행한다. 이 믹스인은 제목 + "곧 채워집니다" 자리표시자만 그리는
공용 골격을 제공해, 세 섹션이 사이드바에서 열리고 닫히는 경로(show_section, 지연
생성)를 지금 확정한다.
...
PLACEHOLDER_TITLES: dict[str, tuple[str, str]] = {
    ...
    "alarms": ("detail.nav.alarms", "알람"),
    "settings": ("detail.nav.settings", "설정"),
}


class HubPlaceholderMixin:
    """Today/알람/설정 빈 골격 섹션의 상단바·본문을 담당합니다."""
